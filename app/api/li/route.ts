...
export async function GET() {
  return NextResponse.json({
    message: 'Li Analysis API is running',
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Li Analysis Request:', body);
    ...
    const data = {
      ...
      analysis_type: 'feifei_li_perspective',
      status: 'received'
    };
    ...
    return NextResponse.json({
      success: true,
      message: 'Li analysis request received successfully',
    ...
    console.error('Li Analysis API Error:', error);
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    console.log('Li Analysis Update Request:', body);
    ...
    return NextResponse.json({
      success: true,
      message: 'Li analysis request updated successfully',
      data: updatedData
    });

  } catch (error) {
    console.error('Li Analysis Update Error:', error);
    return NextResponse.json(
      { error: 'Invalid JSON payload' },
      { status: 400 }
    );
